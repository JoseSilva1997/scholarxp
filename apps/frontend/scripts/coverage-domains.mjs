// Aggregates Vitest's Istanbul coverage JSON into ScholarXP frontend domain folders.
...
const coveragePath = path.resolve(frontendRoot, 'coverage/coverage-final.json');
...
const readCoverage = async () => {
  try {
    ...
    if (error.code === 'ENOENT') {
      console.error(
        'Coverage data not found. Run `pnpm --filter frontend test:coverage` first.',
      );
      process.exit(1);
    }
