...
describe('api client', () => {
  ...
  describe('CSRF token handling', () => {
    ...
    it('updates CSRF token from response headers', async () => {
      ...
      const { apiFetch, setCsrfToken } = await import('@/shared/api/client');
    ...
    it('clears CSRF token with clearCsrfToken function', async () => {
      ...
      const { apiFetch, clearCsrfToken } = await import('@/shared/api/client');
    ...
    it('refreshCsrfToken clears and refetches the token with explicit baseUrl', async () => {
      ...
      const { refreshCsrfToken, setCsrfToken } = await import('@/shared/api/client');
    ...
    it('ensureCsrfToken does not fetch if token is already set', async () => {
      ...
      const { ensureCsrfToken, setCsrfToken } = await import('@/shared/api/client');
    ...
    it('ensureCsrfToken fetches token if not already set (explicit baseUrl)', async () => {
      ...
      const { ensureCsrfToken, clearCsrfToken } = await import('@/shared/api/client');
  ...
  describe('Initialization', () => {
    ...
    it('throws error when VITE_API_URL environment variable is not set', async () => {
      ...
      await expect(import('@/shared/api/client')).rejects.toMatchObject({
