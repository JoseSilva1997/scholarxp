...
export class DailyQuestService {
  ...
  async listHistoryForUser(
    ...
    const masterQuestRows = rows.filter(
      (row) => row.type === QuestTypeValues.masterDailyQuests,
    );
    const earliestQuestDayUtc = pagedQuestDays[pagedQuestDays.length - 1];
    const latestQuestDayUtc = pagedQuestDays[0];
    const completedMasterQuestRows =
      masterQuestRows.length === 0
        ? []
        : await this.prisma.dailyQuest.findMany({
            where: {
              ...
              questDateUtc: {
                gte: new Date(
                  earliestQuestDayUtc.getTime() -
                    MASTER_QUEST_STREAK_MAX * UTC_DAY_IN_MS,
                ),
                lt: new Date(latestQuestDayUtc.getTime() + UTC_DAY_IN_MS),
              },
            },
            select: {
              questDateUtc: true,
    ...
    const completedMasterQuestDayKeys = new Set(
      completedMasterQuestRows.map((row) =>
        row.questDateUtc.toISOString().slice(0, 10),
      ),
    );
    const completedMasterQuestIds = masterQuestRows
      .filter((row) => row.isCompleted)
      .map((row) => row.id);
    const awardedExpByQuestId =
      completedMasterQuestIds.length === 0
        ? new Map<number, number>()
        : new Map(
            (
              await this.prisma.expLedger.findMany({
                where: {
                  questId: {
                    in: completedMasterQuestIds,
                  },
              ...
              entry.questId === null ? [] : [[entry.questId, entry.awardedExp]],
            ),
          );
      ...
      quests: rows.map((row): QuestHistoryItemDto => {
        ...
        const rewardBreakdown =
          questType === QuestTypeValues.masterDailyQuests
            ? buildMasterQuestRewardBreakdown({
                baseExp: row.expGranted,
                questDayUtc,
                isCompleted: row.isCompleted,
                awardedExp:
                  awardedExpByQuestId.get(row.id) ??
                  (row.isCompleted ? row.expGranted : null),
                completedMasterQuestDayKeys,
              })
            : null;
        ...
        return {
          ...
          expGranted: rewardBreakdown?.totalExp ?? row.expGranted,
          isCompleted: row.isCompleted,
...
function buildMasterQuestRewardBreakdown(input: {
  ...
  questDayUtc: string;
  isCompleted: boolean;
  awardedExp: number | null;
  completedMasterQuestDayKeys: Set<string>;
}): QuestRewardBreakdown {
  if (input.isCompleted) {
    const totalExp = input.awardedExp ?? input.baseExp;
    return {
      baseExp: input.baseExp,
      streakBonusExp: Math.max(0, totalExp - input.baseExp),
      totalExp,
    };
  }

  const streakBonusExp = calculateProjectedMasterQuestStreakBonusExp(
    input.questDayUtc,
    input.completedMasterQuestDayKeys,
  );

  return {
    baseExp: input.baseExp,
    streakBonusExp,
    totalExp: input.baseExp + streakBonusExp,
  };
}
...
function calculateProjectedMasterQuestStreakBonusExp(
  ...
  const questDayStartUtc = new Date(`${questDayUtc}T00:00:00.000Z`);
  let streakCount = 0;

  for (
    let dayOffset = 1;
    dayOffset <= MASTER_QUEST_STREAK_MAX;
    dayOffset += 1
  ) {
    const priorDayKey = new Date(
      questDayStartUtc.getTime() - dayOffset * UTC_DAY_IN_MS,
    )
      .toISOString()
      .slice(0, 10);
    if (!completedMasterQuestDayKeys.has(priorDayKey)) {
      break;
    }
  ...
  return (
    Math.round(MASTER_QUEST_COMPLETION_REWARD * (1 + bonusPercent / 100)) -
    MASTER_QUEST_COMPLETION_REWARD
  );
}
