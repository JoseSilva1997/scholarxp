    ...
    return {
      quests: rows.map((row): QuestHistoryItemDto => {
        ...
        return {
          ...
          progressCurrent: row.isCompleted
...
function buildQuestDescription(
  ...
  if (type === QuestTypeValues.moduleUnitRetry) {
    if (moduleUnitTitle) {
      return `Re-complete all questions from the '${moduleUnitTitle}' lesson in the ${moduleTitle} module.`;
    }
    return `Re-complete a lesson that you already mastered in your ${moduleTitle} module.`;
  }
  if (type === QuestTypeValues.dailyPracticeStreak) {
    return 'Reach a streak of 3 correct answers during a daily practice set.';
  }
